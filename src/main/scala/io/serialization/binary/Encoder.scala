 ...
 * Each run can define a table order:
 * [table_id1: u8][table_id2: u8], ..., [table_idn: u8]
 * To then simply save the values
 * [value1: 0-65535 bytes][value2: 0-65535 bytes], ..., [valuen: 0-65535 bytes]
 * Or by single entry:
...
class Encoder(capacity: Int = 15) { 
    ...
    def encodeWithOrder(value: AnyVal): Unit = {
        value match {
            case f: Float => buffer.putFloat(f)
            case i: Int => buffer.putInt(i)
        }
