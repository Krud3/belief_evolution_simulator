...
"
q0_res <- as.data.table(dbGetQuery(conn, q0))
current_network <- ""
...
network_structure <- as.data.table(dbGetQuery(conn, q2))
agents <- as.data.table(dbGetQuery(conn, q3))


dbDisconnect(conn)

data_table_result <- merge(data_table_result, agents[, .(id, tolerance_radius, name)], by.x = "agent_id", by.y = "id")
